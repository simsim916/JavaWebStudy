...
<table border="1" style="width:100%">
	<tr bgcolor="Aquamarine">
		...
		<th>Image</th><th>비고</th>
	</tr>
	
<c:if test="${!empty requestScope.banana }">
	<c:forEach var="b" items="${requestScope.banana}">
		<!-- idbList : id별 boardList
			=> 선택된 id를 function 에 전달
			 -->
		<tr>
			<td onclick="idbList('${b.id}')">${b.id}</td><%-- <td>${b.password}</td> --%><td>${b.name}</td>
			<td>${b.age}</td><td onmousemove="showMemberList(event,'${b.jno}')">${b.jno}<div style="position:fixed "></div></td><td>${b.info}</td>
 ...
 <!--  ** Delete 기능 추가 
			...
			<td><span class="textlink" onclick="axiDelete('${b.id}')">Delete</span></td>
		</tr>
	</c:forEach>
</c:if>
<c:if test="${empty requestScope.banana }">
